...
function App() {
	...
	const dimension =
		dimensions.find((item) => item.key === dimensionKey) ?? dimensions[0];
	...
	const draggingLayer = drag
		? layers.find((layer) => layer.id === drag.id)
		: null;
	const initialBackground = dimension.backgrounds[0];
		...
		drag?.mode === "move" && draggingLayer
			? {
					x:
						Math.abs(
							draggingLayer.x +
								draggingLayer.width / 2 -
								dimension.size.width / 2,
						) <= guideThreshold,
					y:
						Math.abs(
							draggingLayer.y +
								draggingLayer.height / 2 -
	...
	const scale = useMemo(() => {
		...
		const maxWidth = Math.max(
			260,
			(workspaceFitSize.width || viewport.width) - (isMobile ? 32 : 64),
		);
		const maxHeight = Math.max(
			260,
			isMobile ? viewport.height * 0.68 : viewport.height - 230,
		);
		return Math.min(
			maxWidth / dimension.size.width,
			maxHeight / dimension.size.height,
			1,
		);
	}, [
	...
	useEffect(() => {
		const onKeyDown = (event: KeyboardEvent) => {
			...
			setLayers((current) =>
				current.filter((layer) => layer.id !== selectedId),
			);
			setSelectedId(null);
		};
		...
		if (drag.mode === "move") {
			updateLayer(drag.id, {
				x: Math.round(
					Math.max(
						-drag.layer.width,
						Math.min(dimension.size.width, drag.layer.x + dx),
					),
				),
				y: Math.round(
					Math.max(
						-drag.layer.height,
						Math.min(dimension.size.height, drag.layer.y + dy),
					),
				),
			});
			return;
	...
	return (
		<div className="min-h-screen bg-(--user-bg) text-(--user-text)">
			<header className="border-b border-(--user-border) bg-white/90 px-5 py-4 backdrop-blur">
				<div className="flex w-full items-center justify-between gap-4">
					<div className="flex items-center gap-3">
						<img
							className="h-10 w-10 rounded-xl bg-(--app-accent) p-1.5"
							src={asset("icon_transparent.png")}
							alt=""
						/>
						<div>
							<p className="text-[10px] font-bold uppercase tracking-[0.16em] text-(--user-subtle)">
								Manoosia
							</p>
			...
			<main className="flex h-[calc(100svh-73px)] flex-col gap-4 overflow-hidden px-3 py-3 sm:px-4 sm:py-4 lg:grid lg:h-[calc(100vh-73px)] lg:grid-cols-[280px_minmax(0,1fr)_320px]">
				<details
					className="mobile-disclosure panel order-2 overflow-hidden lg:order-none"
					open
				>
							...
							<div className="thumb-grid">
								{dimension.backgrounds.map((item) => (
									<button
										key={item}
										className={
											item === background ? "thumb thumb-active" : "thumb"
										}
										type="button"
										onClick={() => setBackground(item)}
									>
										<img
											src={asset(`background/${dimension.key}/${item}`)}
											alt=""
										/>
									</button>
								))}
						...
						<Section title="Characters">
							<div className="thumb-grid">
								{characters.map((item) => (
									<button
										key={item}
										className="thumb character-thumb"
										type="button"
					...
					<div className="workspace-fit" ref={workspaceFitRef}>
						...
						>
							...
							{layers.map((layer) => (
								<fieldset
									key={layer.id}
									aria-label={`${layer.kind} layer`}
									className={
										layer.id === selectedId ? "layer layer-selected" : "layer"
									}
									style={{
										left: layer.x * scale,
										top: layer.y * scale,
				...
				<details
					className="mobile-disclosure panel order-3 overflow-hidden lg:order-none"
					open
				>
					...
					<div className="mobile-disclosure-body">
						...
						{selectedLayer?.kind === "text" ? (
							<Section title="Text">
								<label className="label-xs" htmlFor="selected-text-content">
									Content
								</label>
								<textarea
								...
								/>
								<label className="label-xs mt-3" htmlFor="selected-text-font">
									Font
								</label>
								<select
								...
								<div className="label-xs mt-3">Color</div>
								...
								/>
								<div className="label-xs mt-3">Text Mode</div>
										...
										<button
											...
											className={
												selectedLayer.align === align
													? "choice choice-active justify-center text-center capitalize"
													: "choice justify-center text-center capitalize"
											}
											type="button"
							...
							<p className="rounded-2xl bg-(--user-surface-soft) p-4 text-sm leading-6 text-(--user-muted)">
								Select layer to reorder. Select text layer to edit font and
								content.
							</p>
						)}
					</div>
...
function drawWrappedText(context: CanvasRenderingContext2D, layer: TextLayer) {
	...
	lines
		.slice(0, Math.floor(layer.height / lineHeight))
		.forEach((text, index) => {
...
function fitTextLayer(layer: TextLayer, size: Dimension["size"]) {
	...
	const width = Math.min(
		size.width * 1.6,
		Math.max(60, Math.ceil(textWidth + layer.fontSize * 0.2)),
	);
	const height = Math.min(
		size.height * 1.6,
		Math.max(40, Math.ceil(lines.length * lineHeight)),
	);
	return resizeFromCenter(layer, Math.round(width), Math.round(height));
}

function resizeFromCenter<T extends Layer>(
	layer: T,
	width: number,
	height: number,
): T {
